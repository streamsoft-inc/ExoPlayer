 ...
 */
public final class MpeghAudioSink implements AudioSink {
  ...
  public static class DefaultAudioProcessorChain implements AudioProcessorChain {
    ...
    public DefaultAudioProcessorChain(
      ...
      this.audioProcessors = new AudioProcessor[audioProcessors.length];
      System.arraycopy(
          /* src= */ audioProcessors,
          /* srcPos= */ 0,
    ...
    public PlaybackParameters applyPlaybackParameters(PlaybackParameters playbackParameters) {
      return new PlaybackParameters(1f, 1f);
    }
   ...
   */
  public MpeghAudioSink(
      AudioProcessorChain audioProcessorChain,
      String appRootPath,
      boolean enableAudioTrackPlaybackParams,
      boolean enableOffload) {
    this.audioProcessorChain = Assertions.checkNotNull(audioProcessorChain);
    this.enableFloatOutput = true;
    ...
    toIntPcmAvailableAudioProcessors = toIntPcmAudioProcessors.toArray(new AudioProcessor[toIntPcmAudioProcessors.size()]);
    volume = 1f;
  ...
  public int getFormatSupport(Format format) {
    ...
    if (isPassthroughPlaybackSupported(format, null)) {
      return SINK_FORMAT_SUPPORTED_DIRECTLY;
    }
    return SINK_FORMAT_UNSUPPORTED;
  }
  ...
  public void configure(Format inputFormat, int specifiedBufferSize, @Nullable int[] outputChannels)
    ...
    if (MimeTypes.AUDIO_RAW.equals(inputFormat.sampleMimeType)) {
      ...
      availableAudioProcessors = toIntPcmAvailableAudioProcessors;
  ...
  private void setupAudioProcessors(boolean isPseudoFlush) {
  ...
  private void pseudoFlushAudioProcessors() {
    for (int i = 0; i < activeAudioProcessors.length; i++) {
      AudioProcessor audioProcessor = activeAudioProcessors[i];
      if (trimmingAudioProcessor.equals(audioProcessor)) {
        audioProcessor.flush();
      }
      outputBuffers[i] = audioProcessor.getOutput();
    }
  ...
  public boolean handleBuffer(
    ...
    if (pendingConfiguration != null) {
      ...
      // [Fix:Gapless]
      // Specify pseudo-Flush to not clear the buffer of AudioProcess.
      applyAudioProcessorPlaybackParametersAndSkipSilence(presentationTimeUs, true);
    }
    ...
    if (startMediaTimeUsNeedsInit) {
      ...
      applyAudioProcessorPlaybackParametersAndSkipSilence(presentationTimeUs, false);
  ...
  public void reset() {
    // [Fix:Gapless]
    // Since it is called at the end of Codec, switch notification is performed for the termination process of Virtualizer.
  ...
  private void applyAudioProcessorPlaybackParametersAndSkipSilence(long presentationTimeUs, boolean isPseudoFlush) {
    ...
    setupAudioProcessors(isPseudoFlush);
    if (listener != null) {
  ...
  private final class StreamEventCallbackV29 extends AudioTrack.StreamEventCallback {
    ...
    public void onDataRequest(AudioTrack track, int size) {
      Assertions.checkState(track == MpeghAudioSink.this.audioTrack);
      if (listener != null) {
  ...
  private void drainBufferToEndOfStream() {
    // When disabe() of DecoderAudioRenderer occurs due to song feed,
    // the request is accompanied by Flush(),
    // so the end track cannot be reproduced unless AudioTrack is regenerated here.
    boolean isInit = isAudioTrackInitialized();
    try {
      virtualizerProcessor.endOfCodec();
      if (!isInit) {
        initializeAudioTrack();
      }
      processBuffers(lastFeedElapsedRealtimeMs*1000);
