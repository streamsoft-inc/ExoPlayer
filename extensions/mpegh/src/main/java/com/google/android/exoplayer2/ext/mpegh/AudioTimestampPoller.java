...
/* package */ final class AudioTimestampPoller {
  ...
  private @AudioTimestampPoller.State
  int state;
  ...
  public AudioTimestampPoller(AudioTrack audioTrack) {
    if (Util.SDK_INT >= 19) {
      audioTimestamp = new AudioTimestampPoller.AudioTimestampV19(audioTrack);
      reset();
    } else {
      audioTimestamp = null;
  ...
  private void updateState(@AudioTimestampPoller.State int state) {
    this.state = state;
    switch (state) {
      case STATE_INITIALIZING:
